...
Gap.prototype.startAdvertisingWithManufacturerData = function (manufacturerData) {
    console.log('startAdvertisingWithManufacturerData: ' + manufacturerData);

    var dataBytes = Buffer.from(manufacturerData, 'hex');
    console.log(dataBytes);

    var advertisementDataLength = 3 + 2 + dataBytes.length;
    var advertisementData = Buffer.alloc(advertisementDataLength);

    // flags
    advertisementData.writeUInt8(2, 0);
    advertisementData.writeUInt8(0x01, 1);
    advertisementData.writeUInt8(0x02, 2);
    advertisementData.writeUInt8(dataBytes.length + 1, 3);
    advertisementData.writeUInt8(0xff, 4);

    var i = 0;
    for (var i = 0; i < dataBytes.length; i++) {
        advertisementData.writeUInt8(dataBytes[i], 5 + i);
    }

    var error = null;
    if (advertisementData.length > 31) {
        error = new Error('Advertisement data is over maximum limit of 31 bytes: ' + advertisementData.length);
    }
